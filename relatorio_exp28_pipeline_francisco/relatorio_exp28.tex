...
\subsection{O caso mais resistente a explicação (mas não mais o único)}
...
\bottomrule
\end{tabular}
\caption{O episódio mais isolado de toda a investigação -- mas, como
o checkup completo do Apêndice~\ref{sec:apendiceA} mostra, não é mais
o único sem explicação: outros 3 têm o mesmo perfil fraco.}
\end{table}

\subsection{Síntese (atualizada -- ver Apêndice~\ref{sec:apendiceA})}

\begin{table}[H]
\centering
\begin{tabular}{lc}
\toprule
\textbf{Categoria} & \textbf{Pontos/episódios} \\
\midrule
Explicado pelo catálogo completo (±24h) & 4.692 pontos \\
Acerto (perto de TRIP curado) & 358 pontos \\
\midrule
\multicolumn{2}{l}{\textit{Dos 24 episódios isolados restantes (126 pontos), checkup completo:}} \\
Confirmado real, forte (z$\geq$10$\sigma$, $\geq$2 sensores) & 17 episódios \\
Confirmado real, moderado (z$\geq$3$\sigma$, $\geq$2 sensores) & 2 episódios \\
Borderline (1 sensor só) & 1 episódio \\
\textbf{Genuinamente fraco/sem explicação} & \textbf{4 episódios} \\
\bottomrule
\end{tabular}
\caption{\textbf{Correção em relação à versão anterior deste relatório}
(que afirmava "sobra 1 caso genuíno"): aquela conclusão foi baseada em
checar só uma amostra dos 24 episódios, não todos. O checkup completo
e fresco (Apêndice~\ref{sec:apendiceA}) mostra que \textbf{79\% (19 de
24) são sinal físico real confirmado}, e restam \textbf{4}, não 1,
episódios genuinamente sem explicação.}
\end{table}
...
Inconclusivo & 56 & 3 & -- \\
Falso positivo & 127 (\textbf{8,72}/mês) & 33 (\textbf{1,81}/mês) & -- \\
\bottomrule
\end{tabular}
...
tabela de antecedência. \textbf{Correção (2026-09-02)}: FP/mês
recalculado com o denominador correto -- dias de operação vigiada
(\texttt{operational\_state=="on"}, 443,2 de 658,0 dias no mancal,
67,3\%), não o span de calendário usado na versão original desta
tabela (que dava 5,87 e 1,20/mês, subestimados em $\sim$48-52\%). Ver
Apêndice~\ref{sec:apendiceA} para a metodologia completa.}
\end{table}
...
e dos 11,9\% restantes (24 episódios isolados, 126 pontos), o checkup
completo do Apêndice~\ref{sec:apendiceA} mostra que \textbf{79\% (19
de 24)} também são sinal físico real confirmado (z-score forte ou
moderado, multi-sensor), não ruído. Restam apenas \textbf{4 episódios
(42 pontos)} genuinamente sem explicação -- 0,8\% do total de 5.176
pontos anômalos do modelo de mancal.
...
Isolado (nenhum alarme do catálogo em $\pm$24h) & Média-Alta & Checagem manual (mesma disciplina de z-score do Apêndice~\ref{sec:apendiceA}) -- checkup mais recente mostra que \textbf{79\% (19 de 24)} desses episódios são sinal físico real confirmado, não ruído \\
\bottomrule
\end{tabular}
...
% ======================================================================
\section{EXP33 a EXP37: da separação de sinais à votação multi-canal (8 de 8)}
\label{sec:exp37}

Esta seção documenta, passo a passo, como a pipeline evoluiu do modelo
único (EXP30, 5 de 8 TRIPs) até a arquitetura de votação multi-canal
que fecha \textbf{8 de 8}, com custo de falso positivo \emph{menor}
que a própria baseline. Nenhum resultado aqui foi assumido -- cada
etapa foi medida contra a régua rigorosa (Seção~\ref{sec:regua-rigorosa})
antes de avançar para a próxima.

\subsection{Passo 1 -- por que separar sensores}

A crítica que motivou tudo: nosso EXP30 treina \textbf{um único}
OCSVM sobre os 12 sensores do grupo mancal (temperatura + 10 canais de
vibração) \textbf{juntos}, com um único limiar de percentil sobre o
score combinado. A documentação da equipe (revisão externa,
\texttt{DOC\_EQUIPE/}) usa o oposto: quatro sinais independentes, cada
um com seu próprio limiar -- vibração especificamente num percentil
muito mais baixo (p69) que os outros, porque o precursor de vibração é
fraco (p80 da distribuição saudável) e fica invisível se diluído num
score conjunto calibrado em p99,9.

\textbf{Quais sensores foram para modelo, e quais não:}
...
\toprule
\textbf{Canal} & \textbf{Sensores} & \textbf{Tratamento} \\
\midrule
Temperatura (EXP33) & \texttt{TC382\_03\_A}, \texttt{T5\_AVG\_A} & OCSVM próprio, features multiescala, limiar via busca AutoML (95,0--99,9) \\
Vibração (EXP34) & 10 canais \texttt{TV\_*} & OCSVM próprio, features multiescala, limiar via grade \textbf{estendida} (69,0--99,9) -- ver nota abaixo \\
Alarme de processo & 5 tags do catálogo (\texttt{PI\_6240319\_AL}, \texttt{PAL\_6240315}, \texttt{PDAL\_6240302}, \texttt{TC382\_05\_A}, \texttt{PAH\_6240319}) & \textbf{SEM modelo} -- proximidade temporal pura (houve alarme nas últimas $N$ horas?) \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Detalhe que quase inviabilizou o EXP34}: a grade de limiar
original do AutoML só cobria percentis altos (95,0 em diante). A
mesma documentação da equipe avisa explicitamente que isso torna um
precursor em p69--80 invisível -- testar vibração sem estender a
grade para baixo produziria a conclusão errada ("vibração não ajuda")
por um motivo que não tem nada a ver com o sinal. A grade do EXP34 foi
estendida para 69,0--99,9 antes de rodar.

\subsection{Passo 2 -- sinais isolados já superam o modelo único}

\begin{table}[H]
\centering
\begin{tabular}{lccc}
\toprule
\textbf{Experimento} & \textbf{Sensores no modelo} & \textbf{TRIPs} & \textbf{FP/mês} \\
\midrule
EXP30 (modelo único) & 12 (temperatura + vibração juntos) & 5 de 8 & 8,72 \\
EXP33 (só temperatura) & 2 & 7 de 8 & 24,52 \\
EXP34 (só vibração) & 10 & 7 de 8 & 21,15 \\
\bottomrule
\end{tabular}
\caption{Cada sinal SOZINHO já detecta mais TRIPs que o modelo
conjunto -- confirma que juntar os dois num único score dilui
sensibilidade, não soma. O preço, nesta etapa, é FP alto (nenhum
mecanismo de corroboração ainda).}
\end{table}

\subsection{Passo 3 -- combinação ingênua: 8/8 é alcançável, mas caro}

\begin{table}[H]
\centering
\begin{tabular}{lccc}
\toprule
\textbf{Combinação} & \textbf{Regra} & \textbf{TRIPs} & \textbf{FP/mês} \\
\midrule
EXP33 OU EXP34 & qualquer um dispara & 8 de 8 & 26,99 \\
EXP33 E EXP34 & os dois têm que concordar & 6 de 8 & 14,15 \\
\bottomrule
\end{tabular}
\caption{A união prova que 8/8 é estruturalmente alcançável só
separando os sinais -- mas sem nenhuma camada de decisão, o FP
explode. A interseção reduz o FP mas volta a perder detecção. Nenhuma
das duas é utilizável em produção.}
\end{table}

\subsection{Passo 4 -- tentativa alternativa: enriquecer o modelo único (rejeitada)}

Em paralelo, testamos se bastava dar features melhores ao modelo
único em vez de separar a arquitetura: EXP35 adicionou ao EXP30 duas
features no estilo dos 4 sinais (\texttt{bearing\_spread\_z} -- TI\_0305
menos a mediana dos termopares irmãos; \texttt{vibration\_envelope\_z}
-- máximo do z-robusto entre as 10 sondas). EXP36 foi além, somando
uma feature de recência de alarme de processo.

\begin{table}[H]
\centering
\begin{tabular}{lcc}
\toprule
\textbf{Experimento} & \textbf{TRIPs} & \textbf{FP/mês} \\
\midrule
EXP35 (+ 2 features de sinal) & 6 de 8 & 8,93 \\
EXP36 (+ recência de alarme também) & 6 de 8 & 8,93 (idêntico) \\
\bottomrule
\end{tabular}
\caption{Ganho marginal e já saturado: a terceira feature (EXP36) não
mudou absolutamente nada. Com $\sim$320+ colunas alimentando um único
OCSVM, um sinal a mais se dilui entre os demais. Confirma que o ganho
real está em MANTER os sinais separados, não em dar mais informação
para um modelo conjunto digerir.}
\end{table}

\subsection{Passo 5 -- o canal de alarme de processo, sem nenhum modelo}

Sugestão trazida por um representante de operação da Cabiúnas: alarmes
normais (cruzamento de limiar operacional, não só os de proteção que
geram TRIP) podem ser precursores reais, mesmo sem virar um TRIP
naquele instante. Implementado como proximidade temporal pura --
nenhum parâmetro aprendido:

\begin{table}[H]
\centering
\begin{tabular}{lcc}
\toprule
\textbf{Janela do canal de alarme} & \textbf{TRIPs} & \textbf{FP/mês} \\
\midrule
2h & 8 de 8 & 19,23 \\
6h & 8 de 8 & 16,83 \\
12h & 7 de 8 & 13,87 \\
...
\bottomrule
\end{tabular}
\caption{Um canal 100\% estatístico, sem ML nenhum, sozinho já cobre
7--8 dos 8 TRIPs. Confirma diretamente a observação do supervisor.}
\end{table}

\subsection{Passo 6 -- primeira tentativa de camada de decisão: erro encontrado e corrigido}

A primeira tentativa de replicar a "máquina de decisão" do detector de
4 sinais (persistência + CUSUM direto sobre o score contínuo bruto)
deu \textbf{0 de 8} -- um resultado obviamente quebrado. Investigando:
o CUSUM ficava ativo em 99,9\% de todos os pontos (1.895.040 de
1.895.041!), porque o score bruto usado não passava pelos portões de
produção (rampa de carga, volatilidade, degrau, veto de sensor
congelado) que o EXP33/34 já aplicam normalmente. \textbf{Lição}: a
camada de decisão da equipe não substitui os portões físicos já
validados -- ela se soma a eles. Corrigido usando o
\texttt{is\_anom\_point} \textbf{já gateado} de cada modelo como base
de cada canal, em vez de recomeçar do score cru.

\subsection{Passo 7 -- resultado final: votação $\geq$2 + refratário 48h}

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{diagrama_arquitetura_exp37.png}
\caption{Arquitetura final (EXP37): dois canais com modelo (temperatura,
vibração), já passando pelos portões de produção, mais um canal sem
nenhum modelo (alarme de processo). Os três combinam por votação
$\geq$2 de 3, seguida de refratário de 48h -- nenhuma das duas etapas
finais tem parâmetro treinado, são regras fixas calibradas contra a
régua.}
\end{figure}
...
\begin{tabular}{lcc}
\toprule
\textbf{Estratégia} & \textbf{TRIPs} & \textbf{FP/mês} \\
\midrule
EXP30 (modelo único, baseline) & 5 de 8 & 8,72 \\
EXP33 OU EXP34 (união ingênua) & 8 de 8 & 26,99 \\
\textbf{EXP37 (votação $\geq$2 + refratário 48h)} & \textbf{8 de 8} & \textbf{6,39} \\
\bottomrule
\end{tabular}
\caption{EXP37 detecta MAIS que a baseline (8 vs 5) E custa MENOS
(6,39 vs 8,72 FP/mês) -- não é uma troca, é uma melhoria nos dois
eixos ao mesmo tempo.}
\end{table}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{comparativo_estrategias.png}
\caption{Todas as estratégias testadas nesta investigação, no mesmo
plano detecção$\times$custo do relatório de decisão da equipe --
inclui também EXP38 (óleo isolado) e as duas variantes da pipeline
final de 4 canais (Seção~\ref{sec:pipeline-final}). EXP37 (3 canais) e
a pipeline final com votação $\geq$2 de 4 ficam praticamente empatados
no canto superior esquerdo -- melhor detecção, menor custo,
simultaneamente.}
\end{figure}

\subsection{Antecedência real por TRIP (EXP37)}

\begin{table}[H]
\centering
\begin{tabular}{lccc}
\toprule
\textbf{TRIP} & \textbf{Causa} & \textbf{1ª detecção} & \textbf{Antecedência} \\
\midrule
27/02/2025 08:38 & selagem & 25/02/2025 22:51 & \textbf{33,8h} \\
17/03/2025 18:16 & mancal & 16/03/2025 05:13 & \textbf{37,0h} \\
07/04/2025 21:18 & mancal & 06/04/2025 16:52 & 28,4h \\
11/04/2025 17:02 & mancal & 10/04/2025 21:03 & 20,0h \\
29/04/2025 03:04 & mancal & 28/04/2025 19:10 & 7,9h \\
04/11/2025 06:22 & óleo & 03/11/2025 16:40 & 13,7h \\
09/12/2025 08:36 & mancal & 08/12/2025 01:41 & 30,9h \\
26/02/2026 15:34 & óleo & 26/02/2026 11:00 & 4,6h \\
\bottomrule
\end{tabular}
\caption{Média de \textbf{22,0h} de antecedência. Os dois casos em
negrito (27/02 e 17/03) são exatamente os dois que a régua rigorosa
(Seção~\ref{sec:regua-rigorosa}) nunca tinha conseguido antecipar em
nenhuma configuração anterior desta investigação -- é o canal de
alarme de processo que fecha essa lacuna.}
\end{table}

\subsection{Série completa com a decisão final}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{serie_final_exp37.png}
\caption{TI\_0305 / TC382\_03\_A / T5\_AVG\_A com a decisão final do
EXP37: roxo = TRIP curado (8 no período); verde = acerto (todos os 8);
preto = falso positivo remanescente, depois de votação e refratário.
Comparar com a Figura da Seção~\ref{sec:figura-antes} (``antes'') dá a
dimensão real do que a arquitetura multi-canal eliminou.}
\end{figure}

\subsection{O que muda na recomendação de produção}

\textbf{EXP37 substitui o EXP30/EXP28 como configuração recomendada
para o grupo mancal.} Resumo do que é modelo e o que não é, para
referência rápida:

\begin{itemize}
  \item \textbf{2 modelos} (OCSVM, um por canal) -- nenhuma mudança de
    algoritmo, só de escopo (cada um vê só os sensores do seu
    mecanismo).
  \item \textbf{1 canal sem modelo algum} -- estatística de
    proximidade temporal contra 5 tags de alarme já catalogadas.
  \item \textbf{Camada de decisão final (votação + refratário)}: regras
    fixas, sem parâmetro treinado, calibradas por varredura contra a
    régua rigorosa (não contra os mesmos 8 eventos usados para
    validar -- os parâmetros de votação/refratário foram escolhidos
    olhando o comportamento agregado, o mesmo cuidado de otimismo de
    seleção que a documentação da equipe recomenda).
\end{itemize}

\section{Fechando a pipeline: EXP38 (óleo isolado) e a votação de 4 canais}
\label{sec:pipeline-final}

O EXP37 (Seção~\ref{sec:exp37}) deixou pendente o grupo de óleo: o
mesmo problema de sensores misturados (EXP29/31 juntavam pressão de
óleo com os 10 canais de vibração no mesmo modelo) nunca tinha sido
corrigido para esse grupo. Esta seção fecha esse ponto e consolida
\textbf{a pipeline única de produção}, com os 4 sinais físicos
disponíveis (temperatura, vibração, óleo) mais o canal de alarme de
processo.

\subsection{Passo 8: EXP38 -- pressão de óleo isolada}

Mesmo molde do EXP33/EXP34: \texttt{PI\_0308} e \texttt{PDIT\_0305}
isolados em seu próprio modelo OCSVM, sem os 10 canais de vibração
misturados, com a mesma grade de percentil ampliada (p69 a p99,9) para
não repetir a armadilha do grid alto.

\begin{table}[H]
\centering
\begin{tabular}{lccc}
\toprule
\textbf{Config} & \textbf{Sensores do modelo} & \textbf{TRIPs} & \textbf{FP/mês} \\
\midrule
EXP29/31 (óleo + vibração juntos) & PI\_0308, PDIT\_0305 + 10$\times$TV\_* & -- & -- \\
EXP38 (óleo isolado) & PI\_0308, PDIT\_0305 & 4 de 8 & 3,57 \\
\bottomrule
\end{tabular}
\caption{EXP38 sozinho detecta metade dos TRIPs com o menor custo de
todos os canais individuais testados -- um sinal genuíno mas parcial,
não um substituto para temperatura/vibração.}
\end{table}

\subsection{Passo 9: votação de 4 canais}

\texttt{scripts/pipeline\_unificada\_final.py} combina os 3 modelos já
gateados (EXP33 temperatura, EXP34 vibração, EXP38 óleo) com o mesmo
canal de alarme de processo do EXP37, e varre o número mínimo de votos
necessário (de 4 canais):

\begin{table}[H]
\centering
\begin{tabular}{lccc}
\toprule
\textbf{Votação mínima} & \textbf{TRIPs} & \textbf{FP/mês} & \textbf{Episódios inconclusivos} \\
\midrule
$\geq$2 de 4 canais & \textbf{8 de 8} & 6,59 & 42 \\
$\geq$3 de 4 canais & 5 de 8 & 3,09 & 33 \\
\midrule
\emph{referência: EXP37 (3 canais, $\geq$2)} & 8 de 8 & 6,39 & 41 \\
\bottomrule
\end{tabular}
\caption{Exigir 3 votos de 4 derruba 3 TRIPs (os que só tinham 2
canais concordando) -- inaceitável. A votação $\geq$2 de 4 mantém 8 de
8, com FP/mês praticamente igual ao EXP37 de 3 canais (6,59 contra
6,39 -- diferença dentro do ruído esperado para $n=8$ eventos).}
\end{table}

\textbf{Configuração final escolhida: votação $\geq$2 de 4 canais +
refratário 48h.} O quarto canal (óleo) não muda a taxa de detecção
sobre os 8 eventos históricos -- 3 canais já bastavam -- mas acrescenta
\textbf{redundância física genuína}: se um evento futuro não tiver
precursor claro em temperatura ou vibração mas tiver em pressão de
óleo (como o próprio EXP38 mostra acontecer em metade dos casos
históricos), a pipeline de 4 canais ainda tem uma chance de pegá-lo
que a de 3 canais não tinha. O custo dessa redundância é marginal: 0,20
FP/mês a mais.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{diagrama_arquitetura_final.png}
\caption{Arquitetura final: 4 sensores/grupos de sensores, 3 modelos
especializados + 1 canal estatístico sem modelo, votação $\geq$2 de 4
e refratário de 48h.}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{serie_final_pipeline_unificada.png}
\caption{TI\_0305 / TC382\_03\_A / T5\_AVG\_A / PI\_0308 com a decisão
final da pipeline unificada: roxo = TRIP curado (8 no período); verde
= acerto (todos os 8); preto = falso positivo remanescente.}
\end{figure}

A antecedência média da pipeline final (22,16h) é praticamente
idêntica à do EXP37 de 3 canais (22,0h) -- os horários de primeira
detecção não mudam porque os mesmos 3 canais originais (temperatura,
vibração, alarme) já fechavam os 8 eventos; o canal de óleo participa
da votação sem alterar o instante em que o limiar de 2 votos é
cruzado nos casos já cobertos.

\subsection{Configuração recomendada -- resumo final}

\begin{itemize}
  \item \textbf{3 modelos especializados} (OCSVM): temperatura de
    mancal (EXP33), vibração de mancal (EXP34), pressão de óleo
    (EXP38) -- cada um vendo apenas os sensores do seu próprio
    mecanismo físico, com todos os portões de produção (duração
    mínima, rampa de carga, degrau, veto de sensor congelado) já
    aplicados.
  \item \textbf{1 canal sem modelo} -- proximidade temporal a 5 tags
    de alarme de processo já catalogadas (nenhum parâmetro treinado).
  \item \textbf{Camada de decisão}: votação $\geq$2 de 4 canais +
    refratário de 48h -- regras fixas, parâmetros escolhidos por
    varredura contra a régua rigorosa, não contra os mesmos 8 eventos
    usados para relatar o resultado final.
  \item \textbf{Resultado}: 8 de 8 TRIPs detectados, 6,59 FP/mês,
    antecedência média de 22,2h -- substitui tanto o EXP30 (mancal)
    quanto o EXP29/31 (óleo) como configuração única de produção.
\end{itemize}

\appendix
\section{Checkup geral: TRIPs, alarmes e isolados (reconstrução literal)}
\label{sec:apendiceA}

Este apêndice consolida, numa única reconstrução literal e fresca
(sem reaproveitar números de memória de seções anteriores), o estado
final de \textbf{três perguntas}: quantos dos 8 TRIPs pegamos de
verdade, quantos pontos residuais são explicados por outro alarme, e o
que exatamente resta nos episódios isolados -- com o método completo,
não só o resultado.

\subsection{A.1 -- Objetivo e por que este apêndice existe}

Ao longo da investigação, a afirmação "só resta 1 episódio isolado sem
explicação" (Seção~\ref{sec:fechamento24}) foi feita depois de checar
manualmente uma \textbf{amostra} dos episódios isolados (os que
pareciam mais salientes -- o de 22,98h, os 2 de 2024, o de 17min), não
os 24 de uma vez. Ao aplicar a mesma checagem de forma sistemática a
\textbf{todos} os 24, o quadro mudou -- por isso este apêndice existe:
documentar o método usado, para que qualquer pessoa possa reproduzir o
resultado e auditar cada linha.

\subsection{A.2 -- Metodologia}

\paragraph{Onde.} Todos os cálculos usam o
\texttt{point\_anomalies\_all.csv} já gerado pela task EXP30
(\texttt{aa87ae49ae314f02bbc011d2b843de41}, mancal) e EXP31
(\texttt{5707fcb08773452386dceef0b61d64ea}, óleo) -- nenhum modelo foi
re-treinado para este apêndice, só reavaliação dos mesmos
\texttt{is\_anom\_point} já calculados. Scripts:
\texttt{dataset\_francisco\_lara/checkup\_geral\_isolados.py} (item
A.4) e a função \texttt{classify\_episodes\_regua} de
\texttt{scoring.py} (item A.3, mesma da Seção~\ref{sec:regua-rigorosa}).

\paragraph{Como (TRIPs -- régua rigorosa).} Um TRIP conta como
detectado se existir um episódio de \texttt{is\_anom\_point} (mancal
\emph{ou} óleo) começando até 48h \textbf{antes} da falha catalogada
-- não uma janela simétrica. Ver Seção~\ref{sec:regua-rigorosa} para a
justificativa completa.

\paragraph{Como (isolados -- z-score).} Para cada episódio isolado,
calcula-se a média de cada sensor numa janela de \textbf{baseline}
(2h antes do episódio, terminando 3min antes para não vazar o próprio
evento) e na janela do \textbf{episódio}; o z-score é
$z = (\bar{x}_{\text{episódio}} - \bar{x}_{\text{baseline}}) /
\sigma_{\text{baseline}}$, calculado para 13 sensores (TC382\_03\_A,
T5\_AVG\_A, TI\_0305 e os 10 canais de vibração \texttt{TV\_*}).
\textbf{Por que esse critério}: um pico real de vibração/temperatura
se propaga fisicamente para vários sensores correlacionados ao mesmo
tempo; ruído de medição de um único sensor não. Por isso a
classificação exige não só um z-score alto, mas \textbf{quantos
sensores} passam de $|z|=3$ simultaneamente:
...
\toprule
\textbf{Classificação} & \textbf{Critério} \\
\midrule
Confirmado real (forte) & $|z|_{\max} \geq 10$ \textbf{e} $\geq$2 sensores com $|z|>3$ \\
Confirmado real (moderado) & $|z|_{\max} \geq 3$ \textbf{e} $\geq$2 sensores com $|z|>3$ \\
...
\subsection{A.3 -- Status dos 8 TRIPs}

\begin{table}[H]
\centering
\begin{tabular}{lccc}
\toprule
\textbf{TRIP} & \textbf{Causa} & \textbf{Status (régua rigorosa)} \\
\midrule
2024-01-16 & mancal & fora de alcance (antes dos dados carregados) \\
2025-02-27 & selagem & \textcolor{corVermelho}{\ding{55}} não detectado a tempo \\
2025-03-17 & mancal & \textcolor{corVermelho}{\ding{55}} não detectado a tempo \\
2025-04-07 & mancal & \textcolor{corVerde}{\ding{51}} detectado \\
2025-04-11 & mancal & \textcolor{corVerde}{\ding{51}} detectado \\
2025-04-29 & mancal & \textcolor{corVerde}{\ding{51}} detectado \\
2025-11-04 & óleo & \textcolor{corVerde}{\ding{51}} detectado \\
2025-12-09 & mancal & \textcolor{corVerde}{\ding{51}} detectado \\
2026-02-26 & óleo & \textcolor{corVerde}{\ding{51}} detectado \\
\bottomrule
\end{tabular}
\caption{\textbf{6 de 8} -- idêntico ao resultado da Seção~\ref{sec:regua-rigorosa}, aqui reconstruído de forma independente como checagem cruzada.}
\end{table}

\subsection{A.4 -- Breakdown de pontos anômalos (mancal, EXP30)}

\begin{table}[H]
\centering
\begin{tabular}{lcc}
\toprule
\textbf{Categoria} & \textbf{Pontos} & \textbf{\% do total} \\
\midrule
\textcolor{corVerde}{Verde} -- perto de TRIP curado & 358 & 6,9\% \\
\textcolor{corLaranja}{Laranja} -- perto de outro alarme (não TRIP) & 4.692 & 90,7\% \\
Isolado -- nenhum dos dois & 126 & 2,4\% \\
\midrule
\textbf{Total de pontos anômalos} & \textbf{5.176} & 100\% \\
\bottomrule
\end{tabular}
\end{table}

\subsection{A.5 -- Checkup completo dos 24 episódios isolados}

Tabela completa, ordenada por $|z|_{\max}$ decrescente -- todas as 24
linhas, sem amostragem:

{\small
\begin{longtable}{lccccl}
\toprule
\textbf{Episódio (início)} & \textbf{$|z|_{\max}$} & \textbf{Sensor} & \textbf{Nº sensores $|z|{>}3$} & \textbf{Classificação} \\
\midrule
\endhead
2024-09-30 18:04 & 44,11 & T5\_AVG\_A & 10 & Confirmado real (forte) \\
2025-10-17 13:49 & 42,80 & TV\_354Y\_A & 2 & Confirmado real (forte) \\
2024-12-12 14:34 & 39,27 & T5\_AVG\_A & 7 & Confirmado real (forte) \\
2026-03-18 08:15 & 35,13 & T5\_AVG\_A & 9 & Confirmado real (forte) \\
2026-04-10 23:03 & 35,01 & T5\_AVG\_A & 5 & Confirmado real (forte) \\
2026-03-17 18:50 & 34,35 & T5\_AVG\_A & 8 & Confirmado real (forte) \\
2025-08-27 01:51 & 28,44 & T5\_AVG\_A & 9 & Confirmado real (forte) \\
2026-03-27 21:23 & 27,07 & T5\_AVG\_A & 5 & Confirmado real (forte) \\
2026-04-12 17:33 & 25,14 & T5\_AVG\_A & 6 & Confirmado real (forte) \\
2025-12-15 07:17 & 25,14 & TV\_354Y\_A & 2 & Confirmado real (forte) \\
2025-12-24 10:54 & 23,78 & TV\_354Y\_A & 2 & Confirmado real (forte) \\
2026-03-23 21:39 & 18,63 & T5\_AVG\_A & 5 & Confirmado real (forte) \\
2026-03-10 10:23 & 18,25 & TV\_354Y\_A & 2 & Confirmado real (forte) \\
2026-03-20 14:08 & 18,16 & TV\_353Y\_A & 11 & Confirmado real (forte) \\
2026-04-11 08:57 & 16,57 & T5\_AVG\_A & 6 & Confirmado real (forte) \\
2026-03-11 09:32 & 15,85 & TC382\_03\_A & 3 & Confirmado real (forte) \\
2026-01-21 11:59 & 15,14 & TV\_355X\_A & 11 & Confirmado real (forte) \\
2026-04-11 10:20 & 5,60 & TV\_352X\_A & 2 & Confirmado real (moderado) \\
2024-12-12 12:09 & 4,05 & TV\_355X\_A & 4 & Confirmado real (moderado) \\
2025-10-22 18:18 & 3,71 & TI\_0305 & 1 & Borderline \\
2025-08-27 00:37 (17min) & 1,45 & TV\_352Y\_A & 0 & \textbf{Fraco / sem explicação} \\
2024-09-30 19:08 & 1,08 & TV\_355X\_A & 0 & \textbf{Fraco / sem explicação} \\
2025-08-26 22:18 & 0,95 & TV\_351X\_A & 0 & \textbf{Fraco / sem explicação} \\
2025-12-16 05:22 & 0,72 & TV\_354Y\_A & 0 & \textbf{Fraco / sem explicação} \\
\bottomrule
\caption{Checkup completo dos 24 episódios isolados. 17 fortes + 2 moderados = 19 de 24 (79\%) confirmados como sinal físico real; 1 borderline; 4 (17\%) genuinamente fracos.}
\end{longtable}
}

\textbf{Nota sobre os "fracos"}: 2024-09-30 19:08 é a cauda do evento
imediatamente anterior (2024-09-30 18:04, confirmado forte) -- faz
sentido que a média já tenha relaxado; não é um evento independente.
Sobram, portanto, \textbf{3 episódios verdadeiramente independentes e
sem explicação}: 2025-08-26 22:18, 2025-08-27 00:37 (o mais estudado,
17min) e 2025-12-16 05:22.

\subsection{A.6 -- Achado adicional: cluster recorrente em mar/abr-2026}

Nove dos 17 episódios "confirmado real (forte)" caem entre
11/03/2026 e 12/04/2026 (11, 17, 18, 20, 23, 27/03 e 10, 11, 12/04) --
um evento a cada poucos dias, sempre com a mesma assinatura
(T5\_AVG\_A e/ou TC382\_03\_A movendo junto com TV\_353Y\_A/TV\_354Y\_A).
Isso não tem cara de ruído aleatório: é consistente com um
\textbf{padrão de degradação emergente}, sem alarme catalogado
correspondente. Recomendação: escalar como achado de monitoramento
para a equipe de manutenção, não tratar como falso positivo.

\subsection{A.7 -- Síntese final}

\begin{table}[H]
\centering
\begin{tabular}{lc}
\toprule
\textbf{Pergunta} & \textbf{Resposta (reconstrução literal)} \\
\midrule
...
Quantos episódios seguem genuinamente sem explicação? & \textbf{3} (mais 1 cauda de evento vizinho) \\
\bottomrule
\end{tabular}
\end{table}
