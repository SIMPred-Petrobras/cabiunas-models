"""Filtro de duracao ADAPTATIVO (nao um numero fixo de minutos): a
duracao minima pra contar como alarme e derivada do proprio "ruido de
fundo" do modelo daquele mes, nao um valor fixo calibrado uma vez contra
o modelo congelado.

MOTIVACAO: docs/analise_automl_exp10.md, secao "Filtro de duracao +
walk-forward + veto -- refutado com forca". O filtro fixo de 6min (achado
contra o modelo CONGELADO) derrubou o hit_rate de 92,5% pra 40% quando
combinado com o retreino mensal -- a forma da subida do score perto de
um precursor real muda de mes pra mes (fronteira de decisao diferente a
cada retreino), entao um numero fixo em minutos nao generaliza.

IDEIA (mesma logica ja usada pro LIMIAR de score, aplicada um nivel
acima, pra duracao): o limiar de score em si NUNCA e um numero fixo --
e o percentil 99,9 do erro do PROPRIO treino daquele mes, exatamente
pra se auto-calibrar a cada retreino. Por definicao, ~0,1% dos pontos de
treino cruzam esse limiar -- sao os "falsos positivos" intrinsecos
aquele modelo especifico, ocorrendo em dados 100% normais (o treino
exclui janelas de alarme). A DURACAO desses cruzamentos-de-ruido dentro
do proprio treino e a regua natural de "quanto ruido dura" para aquele
modelo -- se um episodio de avaliacao dura muito mais que isso, e mais
provavel ser sinal real; se dura igual ou menos, e mais provavel ser o
mesmo tipo de ruido que ja existe no treino.

METODO: pra cada mes do walk-forward, alem do fit normal, mede a
duracao dos episodios de cruzamento-de-limiar dentro do PROPRIO treino
(RLE ciente de gaps de tempo -- o treino exclui periodos, entao pontos
vizinhos na tabela podem estar longe no tempo real). Define o limiar de
duracao daquele mes como um percentil alto (ADAPTIVE_PERCENTILE) dessa
distribuicao de ruido -- se nao houver nenhum episodio de ruido no
treino (comum, ja que ~0,1% de pontos raramente formam sequencias),
usa 1 amostra (equivalente a nao filtrar nada extra naquele mes).
Aplica esse limiar SO nos pontos daquele mes (nao concatena entre meses
-- o limiar em si muda mes a mes, entao nao faz sentido medir um
episodio "global" com um corte que varia no meio dele).
...
ADAPTIVE_PERCENTILE = 99.0   # percentil da duracao de ruido do PROPRIO treino usado como corte
...
task = Task.init(
    ...
    task_name="cnn1d-ae::combinado_exp10c_duracao_adaptativa",
    output_uri=True,
    reuse_last_task_id=False,
)
task.set_base_docker(cfg.CLEARML_DOCKER_IMAGE)
task.connect(cfg_dict)
task.connect({"frozen_window_min": FROZEN_WINDOW_MIN, "adaptive_percentile": ADAPTIVE_PERCENTILE}, name="combo_params")
...
for month_start, month_end in month_bounds:
    ...
    # --- ruido intrinseco do PROPRIO treino deste mes ---
    noise_flags = wf_model["train_err"] > wf_model["threshold"]
    noise_runs = runs_time_aware(noise_flags, df_normal_wf.index, dt_seconds)
    ...
    if noise_durations_min:
        ...
        cutoff_min = 0.0  # sem nenhum episodio de ruido no treino -- nao filtra nada extra
    ...
    print(f"[ADAPT mes {month_start.date()}] n_fit={wf_model['n_fit']} flags_ativas={int(flags.sum())}  "
          f"ruido_treino: {len(noise_runs)} episodios, corte_adaptativo={cutoff_min:.2f}min "
          f"({cutoff_samples} amostras)  flags_pos_filtro={int(flags_filtered.sum())}", flush=True)
...
result_f = eval_flags(flags_f_full, oos_index, f"F = D + filtro de duracao ADAPTATIVO (p{ADAPTIVE_PERCENTILE} do ruido de treino de cada mes) [COMBO FINAL]")

report = {
    "group": group_name,
    "params": {"frozen_window_min": FROZEN_WINDOW_MIN, "adaptive_percentile": ADAPTIVE_PERCENTILE},
    "D_walkforward_veto": result_d,
    "F_walkforward_veto_duracao_adaptativa": result_f,
    "monthly_cutoffs": monthly_cutoffs,
}
out_path = os.path.join(OUTPUT_DIR, "combinado_exp10c_duracao_adaptativa_result.json")
with open(out_path, "w", encoding="utf-8") as f:
    json.dump(report, f, indent=2, ensure_ascii=False, default=str)
task.upload_artifact("duracao_adaptativa_report_json", artifact_object=out_path)
