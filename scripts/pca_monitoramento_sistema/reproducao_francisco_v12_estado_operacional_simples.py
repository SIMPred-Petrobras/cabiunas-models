"""v12: isola uma variavel suspeita depois do v11 (autoencoder + melhores
hiperparametros da grade dele) ter PIORADO em vez de melhorar em relacao
ao v9, mesmo usando o MESMO ground-truth (9 eventos, mesmas datas,
verificado) que o codigo original dele encontra no nosso dataset.

Achado do v11: nossa reimplementacao performa pior que o codigo dele
independente da arquitetura/hiperparametro testado (PCA-Q no v9,
autoencoder no v11, faixa fisica+veto no v10). Isso sugere causa
estrutural, comum a v6-v11, nao relacionada a escolha de modelo/limiar.

HIPOTESE TESTADA AQUI: nosso criterio de "operacao estavel"
(`build_operational_state`, usado no projeto inteiro -- combina
off_value_quantile, off_abs_threshold, off_long_min_hours,
transient_padding_minutes, transient_diff_quantile) e um algoritmo bem
mais elaborado que a definicao simples dele:

    stable = in_operation (RUNNING_A >= 0.5) E fora das 2h seguintes
             a uma partida (transicao 0->1)

Isso muda a composicao do baseline de treino E quais pontos recebem
pontuacao. v12 e IDENTICO ao v9 (PCA-Q, ewma2h, p99.9, sust30min,
votacao 2-de-4) trocando SO essa definicao, pra isolar o efeito.

Uso:
    PYTHONPATH=. python scripts/pca_monitoramento_sistema/reproducao_francisco_v12_estado_operacional_simples.py
"""
import gc
import os
import sys
...
task = Task.init(
    ...
    task_name="pca-walkforward::monitoramento_sistema_v12_estado_operacional_simples",
    output_uri=True,
...
task.connect(_config, name="reproducao_francisco_v12_config")
...
print("Calculando estado operacional -- definicao SIMPLES dele, nao build_operational_state...", flush=True)
# v12: em vez do nosso build_operational_state (algoritmo mais elaborado,
# usado no resto do projeto: off_value_quantile + off_abs_threshold +
# off_long_min_hours + transient_padding_minutes + transient_diff_quantile),
# usa a definicao dele, bem mais simples:
#   stable = in_operation (RUNNING_A >= 0.5) E fora das STARTUP_EXCLUDE (2h)
#            seguintes a uma partida (transicao 0->1)
STARTUP_EXCLUDE_HOURS = 2.0
starts = is_running_raw & ~is_running_raw.shift(fill_value=False)
startup_window_samples = max(1, int(STARTUP_EXCLUDE_HOURS * 3600 / STEP_SECONDS))
in_startup = starts.rolling(startup_window_samples, min_periods=1).max().astype(bool)
is_on = is_running_raw & ~in_startup
print("fracao 'on' (definicao simples):", is_on.mean(), flush=True)
...
for name, sig in list(sustained.items()) + [("votacao_2de4_v12", alert_votacao)]:
...
print("\nOK - fim do script v12 (estado operacional simples)", flush=True)
task.mark_completed(status_message="v12: teste do criterio de operacao estavel simples (isola variavel do v11) concluido com sucesso.")
task.close()
