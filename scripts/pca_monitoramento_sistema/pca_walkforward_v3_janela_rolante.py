"""v3: mesma base do v2 (pre-processamento real do projeto + avaliacao
por episodio), mas trocando a janela de treino de EXPANSIVA (todo o
historico anterior ao mes avaliado) por ROLANTE FIXA de ~3000h (~125
dias, ~4 meses) -- a mesma ordem de grandeza usada pelo colega, cujo
resultado (0,94 FP/mes) e bem melhor que o nosso com janela expansiva
(3,35-4,42 FP/mes). Hipotese a testar: uma janela mais curta acompanha
melhor a operacao recente da planta (deriva de sensor, mudanca de
setpoint) e gera menos falso positivo por modelo desatualizado.

Unica mudanca estrutural em relacao ao v2: o filtro de treino agora tem
um limite inferior de data (month_start - ROLLING_WINDOW_HOURS), nao so
o limite superior (month_start). Todo o resto (pre-processamento,
PCA, ocsvm/iforest, avaliacao por tag e por episodio) e identico.

Ver docs/analise_pca_monitoramento_sistema.md.

Uso:
    PYTHONPATH=. python scripts/pca_monitoramento_sistema/pca_walkforward_v3_janela_rolante.py
"""
import os
import sys
...
task = Task.init(
    ...
    task_name="pca-walkforward::monitoramento_sistema_v3_janela_rolante",
    output_uri=True,
...
RANDOM_SEED = 42
BURN_IN_MONTHS = 2
ROLLING_WINDOW_HOURS = 3000.0  # ~125 dias / ~4 meses -- igual ordem de grandeza do colega
...
task.connect(_config, name="pca_walkforward_v3_config")
...
for i, m in enumerate(months):
    ...
    window_start = month_start - pd.Timedelta(hours=ROLLING_WINDOW_HOURS)

    train_idx = (
        (df_use.index >= window_start) & (df_use.index < month_start)
        & is_on.values & (~near_any_alarm)
    )
...
for name, flags in [("ocsvm", flags_ocsvm), ("iforest", flags_iforest)]:
    ...
    csv_path = os.path.join(OUTPUT_DIR, f"resultado_v3_{name}_por_tag.csv")
    ...
    task.upload_artifact(name=f"resultado_v3_{name}_por_tag", artifact_object=csv_path)
...
print("\nOK - fim do script v3", flush=True)
task.mark_completed(status_message="Monitoramento PCA walk-forward v3 (janela rolante ~3000h) concluido com sucesso.")
task.close()
