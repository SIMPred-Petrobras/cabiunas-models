"""v7: identico ao v6, exceto por UMA mudanca -- testa a hipotese registrada
no relatorio do v6 (docs/analise_pca_monitoramento_sistema.md e
task_plots_relatorio_v6_reproducao_francisco/) pra explicar por que nossa
cobertura de eventos (2/11) ficou bem abaixo da dele (6/8) mesmo com o FP
replicado quase exatamente (0,90 vs 0,94/mes):

  HIPOTESE: rodamos o PCA-Q das familias 'temperatura'/'pressao_oleo'
  sobre o espaco de FEATURES DERIVADAS COMPLETO do projeto (~196 colunas
  por familia, com medias/desvios/tendencias em 4 janelas), enquanto ele
  roda direto sobre os SENSORES BRUTOS limpos (14/12 colunas). Um espaco
  de entrada de dimensao muito maior pode diluir o erro de reconstrucao
  de um evento pontual de falha.

  MUDANCA: cfg.ENABLE_DERIVED_FEATURES = False (era True no v6). So isso.
  Nenhum outro parametro/logica muda -- se a cobertura melhorar, a causa
  fica isolada.
...
Uso:
    PYTHONPATH=. python scripts/pca_monitoramento_sistema/reproducao_francisco_v7_sem_features_derivadas.py
"""
import os
import sys
...
task = Task.init(
    ...
    task_name="pca-walkforward::monitoramento_sistema_v7_sem_features_derivadas",
    output_uri=True,
...
print("\nOK - fim do script v7 (sem features derivadas)", flush=True)
task.mark_completed(status_message="v7: reproducao do Francisco sem features derivadas concluida com sucesso.")
task.close()
