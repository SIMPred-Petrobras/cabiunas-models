"""v2: mesma ideia do pca_walkforward.py, mas reusando de verdade o
pre-processamento do projeto (clip_outliers, features derivadas via
PipelineConfig/build_group_dataframe/select_feature_columns,
normalize_train_only) em vez de uma reimplementacao manual. Tambem
adiciona avaliacao por EPISODIO (nao por tag individual) -- muitos tags
do catalogo disparam juntos pro mesmo evento fisico (confirmado:
ate 46 dos 47 tags juntos em minutos), entao contar tag a tag infla o
hit_rate agregado. Ver docs/analise_pca_monitoramento_sistema.md.
...
task = Task.init(
    ...
    task_name="pca-walkforward::monitoramento_sistema_v2_preprocessamento_real",
    output_uri=True,
...
task.connect(_config, name="pca_walkforward_v2_config")
...
for name, flags in [("ocsvm", flags_ocsvm), ("iforest", flags_iforest)]:
    ...
    csv_path = os.path.join(OUTPUT_DIR, f"resultado_v2_{name}_por_tag.csv")
    ...
    task.upload_artifact(name=f"resultado_v2_{name}_por_tag", artifact_object=csv_path)
...
print("\nOK - fim do script v2", flush=True)
task.mark_completed(status_message="Monitoramento PCA walk-forward v2 (preprocessamento real) concluido com sucesso.")
task.close()
