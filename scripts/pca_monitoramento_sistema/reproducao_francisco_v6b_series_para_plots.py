"""v6b: identico ao v6 (reproducao_francisco_v6.py), mas alem de imprimir os
resultados TAMBEM exporta como artefatos ClearML as series completas
suavizadas (score/limiar por mes, de cada sinal) e a tabela de eventos
curados com acerto/erro/antecedencia -- dados que o v6 calculava mas nao
salvava, necessarios pra gerar os graficos "acertamos o alarme" do
relatorio LaTeX (fig_serie_completa, fig_zoom_evento_*).
...
task = Task.init(
    ...
    task_name="pca-walkforward::monitoramento_sistema_v6b_series_para_plots",
    output_uri=True,
...
# acumuladores da serie completa (score suavizado + limiar do mes), so p/ os
# graficos do relatorio -- nao influenciam o resultado, so registram o que
# ja foi calculado em cada mes.
signal_score_full = {name: pd.Series(np.nan, index=FULL_INDEX, dtype="float32") for name in SIGNAL_SPECS}
signal_thr_full = {name: pd.Series(np.nan, index=FULL_INDEX, dtype="float32") for name in SIGNAL_SPECS}
...
for i, m in enumerate(months):
    ...
    for name, spec in SIGNAL_SPECS.items():
        ...
        if spec["kind"] == "pca":
            ...
            flag.index = eval_dt
            signal_score_full[name].loc[eval_dt] = s_eval.values.astype("float32")
            signal_thr_full[name].loc[eval_dt] = np.float32(thr)
        else:  # z-robusto
            raw_series = spec["raw"]
            ...
            signal_score_full[name].loc[z_eval.index] = z_eval.values.astype("float32")
            signal_thr_full[name].loc[z_eval.index] = np.float32(Z_THRESHOLD)
...
alert_producao = sustained["temperatura"] & sustained["mancal_spread"]  # inovacao #9: E, nao >=N
...
def evaluate_style_dele(name: str, alert: pd.Series):
    ...
    return {
        ...
        "leads": leads, "episodios": episodios, "fps": fps,
    }


print("\n\n########## RESULTADO ESTILO FRANCISCO (eventos curados) ##########")
res_por_sinal = {}
for name, sig in list(sustained.items()) + [("producao_2sinais_AND", alert_producao)]:
    res = evaluate_style_dele(name, sig)
    res_por_sinal[name] = res
...
# ---------------------------------------------------------- exportacao p/ plots
print("\nExportando series completas e tabela de eventos p/ o relatorio...", flush=True)

export_cols = {
    "is_running": is_running_raw.astype("int8"),
    "is_on": is_on.astype("int8"),
    "scored": scored_mask.astype("int8"),
    "temp_score": signal_score_full["temperatura"],
    "temp_thr": signal_thr_full["temperatura"],
    "temp_alert": sustained["temperatura"].astype("int8"),
    "mancal_z": signal_score_full["mancal_spread"],
    "mancal_thr": signal_thr_full["mancal_spread"],
    "mancal_alert": sustained["mancal_spread"].astype("int8"),
    "selagem_z_smooth": signal_score_full["selagem_z"],
    "selagem_alert": sustained["selagem_z"].astype("int8"),
    "pressao_score": signal_score_full["pressao_oleo"],
    "pressao_thr": signal_thr_full["pressao_oleo"],
    "pressao_alert": sustained["pressao_oleo"].astype("int8"),
    "alert_and_producao": alert_producao.astype("int8"),
    "vote_count": vote_count.astype("int8"),
    "T5_AVG_A_raw": df_temp_use["T5_AVG_A"].astype("float32"),
    "mancal_spread_raw": mancal_spread.astype("float32"),
    "PDIT_0305_raw": selagem_raw.astype("float32"),
}
export_df = pd.DataFrame(export_cols)
export_df = export_df.loc[scored_mask.values]
series_path = os.path.join(OUTPUT_DIR, "series_v6_para_plots.csv.gz")
export_df.to_csv(series_path, index=True, index_label="timestamp", compression="gzip", float_format="%.5g")
print(f"  series exportadas: {export_df.shape} -> {series_path}", flush=True)
task.upload_artifact(name="series_v6_para_plots", artifact_object=series_path)

eventos_rows = []
for ev in events:
    row = {"evento": str(ev["inicio"])}
    for sig_name in ["temperatura", "mancal_spread", "selagem_z", "pressao_oleo", "producao_2sinais_AND"]:
        leads = res_por_sinal[sig_name]["leads"]
        row[f"detectado_{sig_name}"] = str(ev["inicio"]) in leads
        row[f"lead_h_{sig_name}"] = leads.get(str(ev["inicio"]), None)
    eventos_rows.append(row)
eventos_df = pd.DataFrame(eventos_rows)
eventos_path = os.path.join(OUTPUT_DIR, "eventos_v6_hit_miss.csv")
eventos_df.to_csv(eventos_path, index=False)
print(eventos_df.to_string(index=False), flush=True)
task.upload_artifact(name="eventos_v6_hit_miss", artifact_object=eventos_path)

fps_producao_path = os.path.join(OUTPUT_DIR, "fp_producao_v6.csv")
pd.DataFrame({"timestamp": [str(t) for t in res_por_sinal["producao_2sinais_AND"]["fps"]]}).to_csv(fps_producao_path, index=False)
task.upload_artifact(name="fp_producao_v6", artifact_object=fps_producao_path)

print("\nOK - fim do script v6b (series p/ plots)", flush=True)
task.mark_completed(status_message="v6b: series exportadas para o relatorio com sucesso.")
task.close()
