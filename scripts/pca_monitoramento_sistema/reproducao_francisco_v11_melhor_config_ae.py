"""v11: monta a MELHOR config encontrada ate agora, usando o NOSSO
pre-processamento (nao o codigo dele). Base = v9 (formulacao atual,
votacao 2-de-4, ground-truth curado, grade 2min), com os 2 unicos
hiperparametros trocados pelos valores que realmente deram o menor FP
com boa cobertura na grade completa do `policy_sweep`/`arch_sweep`
(rodados com o codigo ORIGINAL dele, sem alteracao -- ver
docs/analise_pca_monitoramento_sistema.md, secoes "Reproducao literal"
e "Repetindo no periodo exato dele"):

    ae|2min|b3000h|excl0d|alm1.0h|ewma30min|q99.97|sust2h|conf2

Diferencas em relacao ao v9:
1. **Arquitetura = autoencoder denso (`MLPRegressor`)**, nao PCA-Q, pros
   2 sinais multivariados (`temperatura`, `pressao_oleo`). Mesma
   estrutura que ele usa: 3 camadas ocultas, gargalo em n/4, ReLU,
   parada antecipada, no maximo 60 epocas. Score = erro quadratico medio
   de reconstrucao (mesmo principio do PCA-Q, versao nao-linear).
2. **EWMA de 30min** (nao 2h) e **sustentacao de 2h** (nao 30min) --
   valores que apareceram repetidas vezes nas linhas de menor FP da
   grade dele (ex.: 5/9 eventos a 0,60 FP/mes no periodo exato dele).
3. **Limiar = percentil 99,97** (nao 99,9) -- mais estrito, corta so
   0,03% do baseline em vez de 0,1%.

Os sinais univariados (`mancal_spread`, `selagem_z`) continuam como
valor bruto + mesmo mecanismo (EWMA -> percentil do proprio baseline),
sem arquitetura nenhuma -- e o que o codigo dele tambem faz (esses 2
sinais nunca passam por PCA nem autoencoder).

Uso:
    PYTHONPATH=. python scripts/pca_monitoramento_sistema/reproducao_francisco_v11_melhor_config_ae.py
"""
import gc
import os
import sys
...
task = Task.init(
    ...
    task_name="pca-walkforward::monitoramento_sistema_v11_melhor_config_ae",
    output_uri=True,
...
GRID_RESAMPLE = "2min"
EWMA_HALFLIFE = "30min"    # v11: 30min (era 2h no v9) -- valor da linha de menor FP na grade dele
THRESHOLD_PCT = 99.97      # v11: percentil mais estrito (era 99.9 no v9)
SUSTAIN_MINUTES = 120      # v11: 2h (era 30min no v9)
CONFIRM_MIN = 2            # votacao N-de-4 generica (igual v9)
...
_config = {
    ...
    "confirm_min": CONFIRM_MIN, "model": "ae",
...
task.connect(_config, name="reproducao_francisco_v11_config")
...
# Os 4 sinais tratados de forma IGUAL: "ae" reduz multivariado via autoencoder;
...
SIGNAL_SPECS = {
    "temperatura": {"kind": "ae", "feat": df_temp_feat},
    "pressao_oleo": {"kind": "ae", "feat": df_press_feat},
...
for i, m in enumerate(months):
    ...
    for name, spec in SIGNAL_SPECS.items():
        if spec["kind"] == "ae":
            ...
            # autoencoder denso: mesma estrutura do dele (gargalo em n/4,
            # ReLU, parada antecipada, no maximo 60 epocas). Alvo = entrada.
            n_features = X_train.shape[1]
            hidden = (max(8, n_features // 2), max(4, n_features // 4), max(8, n_features // 2))
            ae = MLPRegressor(hidden_layer_sizes=hidden, activation="relu",
                              early_stopping=True, n_iter_no_change=5,
                              max_iter=60, random_state=RANDOM_SEED)
            ae.fit(X_train, X_train)
            err_train = np.mean((ae.predict(X_train) - X_train) ** 2, axis=1)
            err_eval = np.mean((ae.predict(X_eval) - X_eval) ** 2, axis=1)

            s_train = pd.Series(err_train, index=train_idx_dt)
            eval_dt = df_feat.index[eval_idx]
            s_eval = pd.Series(err_eval, index=eval_dt)
        else:  # "raw" -- sensor/spread univariado, mesmo mecanismo, sem modelo
            raw_series = spec["raw"]
            s_train = raw_series.loc[train_idx_dt]
            eval_dt = raw_series.loc[eval_idx].index
            s_eval = raw_series.loc[eval_idx]

        # suavizacao (EWMA 30min) aplicada IGUAL ao treino e ao teste, depois
        # limiar = percentil 99,97 do baseline JA suavizado (mesma ordem do dele).
...
for name, sig in list(sustained.items()) + [("votacao_2de4_v11_ae", alert_votacao)]:
...
print("\nOK - fim do script v11 (melhor config: autoencoder)", flush=True)
task.mark_completed(status_message="v11: melhor config (ae, ewma30min, p99.97, sust2h, conf2) concluida com sucesso.")
task.close()
