"""v9: corrige v6/v7/v8 pra bater com a formulacao REAL e ATUAL da pipeline
do Francisco, descrita em `DOCUMENTACAO_DETECTOR_TC33003A.pdf` (documento
de 28/08/2026, mais recente e mais confiavel que o `config.py` antigo que
v6/v7/v8 usaram como referencia). O rotulo da config em producao la e:

    pca|2min|b3000h|excl0d|alm1.0h|ewma2h|q99.9|sust30min|conf2|min0min

Correcoes em relacao ao v7 (que ficou baseado no `config.py` desatualizado):

1. **Confirmacao = votacao 2-de-4 GENERICA**, nao um par fixo
   (temperatura E mancal_spread). Os 4 sinais (temperatura, pressao_oleo,
   mancal_spread, selagem_z) entram como iguais; o alerta dispara quando
   >=2 deles estao ativos ao mesmo tempo -- exatamente o que `Trial.confirm`
   ja fazia no script original dele (`automl_clearml.py`), e o que a nossa
   propria v5 (votacao generica) ja tinha, sem saber que era isso mesmo.
2. **EWMA de 2h pra TODOS os 4 sinais** (nao 1h pros multivariados e
   30min pros univariados como v6/v7/v8 assumiram).
3. **Limiar = percentil 99,9 do score suavizado do proprio baseline,
   aplicado IGUAL pros 4 sinais** -- nao um multiplicador (2,0×p99) pro
   PCA-Q e um z-robusto (|z|>3,0) separado pros univariados. Na pratica
   isso significa que `mancal_spread`/`selagem_z` entram como valor bruto
   (com sinal, sem normalizar por mediana/MAD) suavizado e comparado ao
   percentil do PROPRIO baseline -- mesmo mecanismo do PCA-Q, so que sem
   PCA (a "reconstrucao" de um unico numero e ele mesmo).
4. **Grade reamostrada pra 2 minutos (mediana)** antes de qualquer coisa
   -- v6/v7/v8 nunca fizeram isso, trabalhavam direto na cadencia nativa
   de 30s. Muda `STEP_SECONDS` (30->120), e por consequencia a contagem
   de amostras de `K_WINDOW`/`DEBOUNCE`.
5. **`exclude_alarm_h=1,0h`**: alem de excluir dias antes de eventos
   curados (agora DESLIGADO, `exclude_days=0`, igual a config real),
   exclui do baseline qualquer instante a menos de 1h de QUALQUER
   ativacao de alarme do catalogo (nao so os eventos curados) -- busca
   binaria igual a dele, pra nao virar O(n×m).

O que NAO foi replicado (ainda): o veto de sensor congelado (por familia,
auto-calibrado a 5% de tempo congelado) e a segunda checagem de aprovacao
("detector vivo", silencio maximo). Ver docs/analise_pca_monitoramento_sistema.md.

Uso:
    PYTHONPATH=. python scripts/pca_monitoramento_sistema/reproducao_francisco_v9_formulacao_atual.py
"""
import gc
import os
import sys
...
task = Task.init(
    ...
    task_name="pca-walkforward::monitoramento_sistema_v9_formulacao_atual",
    output_uri=True,
...
task.connect(_config, name="reproducao_francisco_v9_config")
...
print("\nOK - fim do script v9 (formulacao atual)", flush=True)
task.mark_completed(status_message="v9: reproducao da formulacao atual (votacao 2-de-4, EWMA 2h, percentil 99.9, grade 2min) concluida com sucesso.")
task.close()
