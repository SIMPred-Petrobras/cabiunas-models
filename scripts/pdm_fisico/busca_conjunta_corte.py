...
from corte_vetorizado import corta_vet
...
SAIDA = sys.argv[2] if len(sys.argv) > 2 else "busca_conjunta.csv"
...
def main():
    ...
    for kb, kv, ka, h, cr in itertools.product(KB, KV, KA, HC, CR):
        ...
        vt = (np.sum(on, axis=0) >= 2) & mv
        if FRAC > 0:
            # forca do episodio = o canal mais aceso, nas unidades de limiar
            # daquele trial (thr depende de kb/kv)
            F = np.nanmax(np.vstack([(E[c] / (BASE[c] * (kv if c == "vb" else kb))).to_numpy()
                                     for c in SIN]), axis=0)
            vt = corta_vet(vt, F, FRAC)
        voto = pd.Series(vt, index=idx)
        eps0 = episodios(voto)
        for R in RS:
    ...
    T = pd.DataFrame(L); T.to_csv(SAIDA, index=False)
